...
public class TcDataController {
	...
	@RequestMapping(value = REST.SD_TC_DATA, method = RequestMethod.GET)
	public String holiday(Model model) throws ParseException, JsonParseException, 
			JsonMappingException, IOException {
		model.addAttribute("mainMenu", "Static Data  /  Term and Condition Data");
		model.addAttribute("titlePage", "TERM AND CONDITION DATA");
		model.addAttribute("titlePageCreate", "TERM AND CONDITION DATA | NEW");
		model.addAttribute("titlePageEdit", "TERM AND CONDITION DATA | EDIT");
		...
		return HTML.SD_TC_DATA;
	}
	
	@RequestMapping(value = REST.SD_TC_DATA_INQUIRY, method = RequestMethod.GET)
	public @ResponseBody String inquiry(
		...
		String uri = sessionComponent.getHost() + URI.SD_TC_DATA_INQ + "?tenantId=" + sessionComponent.getTenantId();
		ResponseEntity<String> response = restTemplate.exchange(uri, HttpMethod.POST, httpEntity, String.class);
		
		return response.getBody();
	}
	
	@RequestMapping(value = REST.SD_TC_DATA_DROPDOWN_TC_GROUP, method = RequestMethod.GET)
	public @ResponseBody String dropdownTcGroup() {
		return common.dropdownTcGroup();
	}
	
	@RequestMapping(value = REST.SD_TC_DATA_SAVE, method = RequestMethod.POST)
	public @ResponseBody String save(@RequestBody HashMap<String, Object> param) throws JsonProcessingException{
		...
		String uri = sessionComponent.getHost() + URI.SD_TC_DATA_SAVE + "?tenantId=" + sessionComponent.getTenantId();
		ResponseEntity<String> response = restTemplate.exchange(uri, HttpMethod.POST, httpEntity, String.class);
		
		return response.getBody();	
	}
	
	@RequestMapping(value = REST.SD_TC_DATA_DELETE, method = RequestMethod.POST)
	public @ResponseBody String delete(@RequestBody HashMap<String, Object> param) throws JsonProcessingException{
		...
		String uri = sessionComponent.getHost() + URI.SD_TC_DATA_DELETE + "?tenantId=" + sessionComponent.getTenantId();
		ResponseEntity<String> response = restTemplate.exchange(uri, HttpMethod.POST, httpEntity, String.class);
