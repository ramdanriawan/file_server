...
//	public List<TR0012Entity> findAll(Specification<TR0012Entity> spec);
	...
	@Transactional
	@Modifying
	@Query("UPDATE TR0012XEntity tr12 " +
			"SET tr12.trxOfficer = (SELECT saName FROM MA0012Entity WHERE saCode = ?1), " +
			" tr12.trxBranch = (SELECT paChildDesc FROM MA0014Entity WHERE paParentCode = 'OFFCODE' AND " +
								"paChildValue = (SELECT saOfficeId FROM MA0012Entity WHERE saCode = ?1))")
	public void updateTrxBranchAndTrxOfficer(String saCode);
}
